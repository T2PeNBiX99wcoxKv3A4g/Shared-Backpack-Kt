...
object FurnaceCommand {
    ...
    private fun executeFurnace(source: CommandSourceStack, type: FurnaceInventoryType, name: String): Int {
        ...
        when (type) {
            Smelting -> {
                if (!Configs.config.general.sharedSmeltingFurnaceEnabled) {
                    source.sendFailure(
                        Component.translatableWithFallback(
            ...
            Blasting -> {
                if (!Configs.config.general.sharedBlastingFurnaceEnabled) {
                    source.sendFailure(
                        Component.translatableWithFallback(
            ...
            Smoking -> {
                if (!Configs.config.general.sharedSmokingFurnaceEnabled) {
                    source.sendFailure(
                        Component.translatableWithFallback(
